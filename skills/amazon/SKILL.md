...
- 想验证一个市场是否有需求 → 先用 [`google_trends.get_interest_over_time`](../google_trends/SKILL.md) 确认搜索热度，再回来用 `search_products` 评估供给侧
...
```
先用 [Google Trends](../google_trends/SKILL.md) 确认需求信号，再用搜索评估 Amazon 供给侧密度。
