...
| `est_searches_num` | integer | 估算月搜索量 |
| `searches_rank` | integer | 热度排名 |
| `searches` | integer/null | 原始搜索次数 |
| `demand_ratio` | number/null | 需求比率 |
...
| `update_time` | string/null | 数据更新时间 |
...
| `market` | string | 否 | 目标市场：`US`, `UK`, `DE`, `JP`, `CA`, `FR`, `ES`, `IT`；默认 `US` |
| `month` | integer | 否 | 查询最近 N 个月；`-1` 或不传表示全部历史 |
...
- `month` 不是 `-1` 或正整数 → 返回 `invalid_month`
...
- `month` 不传或 `-1` 表示全部历史；传正整数表示最近 N 个月
...
- **star 参数**：只接受 `positive` 或 `negative`，不能传数字
- **only_purchase 参数**：必须传 `true` 或 `false`，字符串 `"true"` 被拒绝
- **start_date 格式**：必须是 `YYYY-MM-DD`
...
- **返回完整路径**：后续调用 `get_category_best_sellers` / `get_category_trend` 时直接复用 `node_id`
...
**CRITICAL — 本能力支持多维度查询，可能出现部分成功。使用前 MUST 理解 [apimux-shared](../apimux-shared/SKILL.md) 中的 Partial-Failure 语义。**
...
- **market 参数**：商品/评论/类目能力需要显式传 market；关键词能力可省略（默认 `US`）
- **ASIN 格式**：10 位大写字母数字，小写会自动转大写
